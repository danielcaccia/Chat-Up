                    ...
                    if let error = error {
                        ...
                        if !querySnapshot!.documents.isEmpty {
                            if let safeDocument = try! querySnapshot!.documents.first!.data(as: User.self) {
                                ...
                                let contact = Contact(with: uid, firstName, lastName, "")
                                let documentRef = self.db.collection(K.Fstore.usersCollectionName).document(self.currentUser!.uid)
                                
                                if !UserSession.shared.contacts.contains(where: { $0.uid == uid}) {
                                    UserSession.shared.contacts.append(contact)
                                }
                                    
                                documentRef.setData([K.Fstore.contactsField: [uid: ""]], merge: true)
                            }
                        } else {
